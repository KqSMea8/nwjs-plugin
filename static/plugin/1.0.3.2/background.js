...
function _backgrountInit() {
    ...
    if(!localStorage["newVersionTips" + jm_VERSION]){
            setTimeout(function(){
                ...
                notification.create("notify_proxy_status", "Icebear白熊求职助手1.0.3.2 版本上线啦! ", "支持最新最热的企业网申, 为你带来前所未有的方便体验.求职路上, 有我陪伴. 详情请猛击…",
                function() {
                chrome.tabs.create({
                    url: "http://icebear.me/"
                    });
...
jm_tools = {
    ...
    init: function() {
        ...
        console.log(isLoadRemoteRes)
        isLoadRemoteRes ? KISSY.io.get("http://ice2018.com/static/chrome_plugin/config.json",
